...
def compute_polar_coordinates(mesh, do_fast=True, radius=12, max_vertices=200, verbose=True):
    """
    ...
    G = vertices_graph(mesh, weighted=True)
...
def compute_theta_all(D, vertices, faces, normals, idx, radius, verbose=True):
    mymds = MDS(n_components=2, n_init=1, max_iter=50, dissimilarity='precomputed', n_jobs=multiprocessing.cpu_count())
    all_theta = []
    for i in range(D.shape[0]):
        if(verbose):
            if i % 1 == 0:
                print('MDS: ', (i + 1) / D.shape[0] * 100, '% done       \r', end='')
...
def compute_theta_all_fast(D, vertices, faces, normals, idx, radius, verbose=True):
    """
    ...
    for i in range(D.shape[0]):
        if(verbose):
            if i % 10 == 0:
                print('MDS_fast: ', (i + 1) / D.shape[0] * 100, '% done       \r', end='')
